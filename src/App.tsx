...
export function App() {
  ...
  async function fetchVibes() {
    try {
      ...
      setLoadError('We could not load the live vibe gallery. Check your connection and try again.');
    } finally {
      setLoading(false);
    }
  }
        ...
        {activeTab === 'gallery' && (
          <div className="space-y-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-[#11111a] rounded-2xl border border-red-900/60">
                <p className="text-red-300 font-mono text-sm">{loadError}</p>
                <button
                  type="button"
                  onClick={fetchVibes}
                  className="mt-4 px-4 py-2 rounded-xl bg-cyan-500 text-black text-sm font-bold hover:bg-cyan-400 transition-colors"
                >
                  Retry loading
                </button>
              </div>
